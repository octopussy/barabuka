    ...
    func attachCallback(on player: AVAudioUnit, audioFormat: AVAudioFormat) {
        if player.auAudioUnit.inputBusses.count > 0 {
            do {
                ...
                UberLog.error("Player auAudioUnit inputbus failure %@",
                ...
                flags _: UnsafeMutablePointer<AudioUnitRenderActionFlags>) -> OSStatus
    {
        ...
        if bytesTotal == 0 {
            ...
            case .cannotDoInCurrentContext:
                UberLog.error("cannotDoInCurrentContext", category: .audioRendering)
                return 0
            case .error:
                UberLog.error("generic error", category: .audioRendering)
                return 0
            @unknown default:
                UberLog.error("unknown error", category: .audioRendering)
                return 0
            }
        }
        return status
