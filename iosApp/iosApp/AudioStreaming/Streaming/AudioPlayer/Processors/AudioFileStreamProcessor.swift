...
final class AudioFileStreamProcessor {
    ...
    func closeFileStreamIfNeeded() {
        guard let fileStream = audioFileStream else {
            UberLog.debug("audio file stream not opened", category: .generic)
            return
        }
    ...
    func processSeek() {
        ...
        if readingEntry.packetDuration > 0, bitrate > 0 {
            ...
            guard seekStatus == noErr else {
                ...
                UberLog.error("seek failed %@", category: .generic, args: streamError.debugDescription)
                return
            }
    ...
    private func processReadyToProducePackets(fileStream: AudioFileStreamID) {
        ...
        let format = playerContext.audioPlayingEntry?.audioStreamFormat
    ...
    func forceDataFormat(audioStreamFormat: AudioStreamBasicDescription) {
        if audioConverter != nil {
            return
        }
        ...
        if !entry.audioStreamState.processedDataFormat {
            ...
            var packetBufferSize: UInt32  = 2048 // default value
            entry.lock.withLock {
            ...
            //processReadyToProducePackets(fileStream: audioFileStream!)
        }
    }
    ...
    func handleEncodedPacket(data: Data) {
        data.withUnsafeBytes { raw in
            let size = UInt32(data.count)
            let desc = AudioStreamPacketDescription(mStartOffset: 0, mVariableFramesInPacket: 0, mDataByteSize: size)
            
            let someClassArg = UnsafeMutablePointer<AudioStreamPacketDescription>.allocate(capacity: 1)
            someClassArg.pointee = desc
            
            propertyPacketsProc(inNumberBytes: size,
                                inNumberPackets: 1,
                                inInputData: raw,
                                inPacketDescriptions: someClassArg)
        }
    }
                             ...
                             inPacketDescriptions: UnsafeMutablePointer<AudioStreamPacketDescription>?)
    {
        ...
        guard let converter = audioConverter else {
            UberLog.error("Couldn't find audio converter", category: .audioRendering)
            return
        }
