...
open class AudioPlayer {
    ...
    public func resume() {
        ...
        serializationQueue.sync {
            do {
                ...
                UberLog.debug("resuming audio engine failed: %@", category: .generic, args: error.localizedDescription)
            }
    ...
    private func setupEngine() {
        do {
            ...
            UberLog.error("⚠️ error setting up audio engine: %@", category: .generic, args: error.localizedDescription)
        }
    }
    ...
    private func startEngineIfNeeded() throws {
        guard !isEngineRunning else {
            UberLog.debug("engine already running 🛵", category: .generic)
            return
        }
        try startEngine()
    }
    ...
    private func startEngine() throws {
        try audioEngine.start()
        UberLog.debug("engine started 🛵", category: .generic)
    }
    ...
    private func pauseEngine() {
        ...
        UberLog.debug("engine paused ⏸", category: .generic)
    }
    ...
    private func stopEngine(reason: AudioPlayerStopReason) {
        ...
        UberLog.debug("engine stopped 🛑", category: .generic)
    }
    ...
    private func setCurrentReading(entry: AudioEntry?, startPlaying: Bool, shouldClearQueue: Bool) {
        ...
        UberLog.debug("Setting current reading entry to: %@", category: .generic, args: entry.debugDescription)
        if startPlaying {
    ...
    private func raiseUnexpected(error: AudioPlayerError) {
        ...
        UberLog.error("Error: %@", category: .generic, args: error.localizedDescription)
    }
}
...
extension AudioPlayer: AudioStreamSourceDelegate {
    func dataAvailable(source: CoreAudioStreamSource, data: Data) {
        ...
        let fmt = source.forcedSourceAudioFormat
        if fmt != nil {
            fileStreamProcessor.forceDataFormat(audioStreamFormat: fmt!)
            fileStreamProcessor.handleEncodedPacket(data: data)
        } else {
